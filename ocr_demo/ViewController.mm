...
//    cv::print(cv::format(originImage, cv::Formatter::FMT_NUMPY));
//    cv::print(cv::format(image, cv::Formatter::FMT_NUMPY));
//    printf("type = %d", image.type());
//    printf("%d, %d, %d, %d, %d, %d, %d", image.at<uchar>(0, 0), image.at<uchar>(0, 1), image.at<uchar>(0, 2),image.at<uchar>(186, 0), image.at<uchar>(186, 1), image.at<uchar>(186, 2),image.at<uchar>(186, 899));
    cv::Mat img;
    image.copyTo(img);
    //打印矩阵
...
//    cv::print(cv::format(img_fp, cv::Formatter::FMT_NUMPY));
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    BLAOCRPPredictor *predictor = [BLAOCRPPredictor defaultPredictor];
    [predictor inferImage:self.image];
...
- (void)viewDidLoad {
    ...
    _image = [UIImage imageNamed:@"0.jpg"];
    if (_image != nil) {
