...
OCR_PPredictor::infer_ocr(const std::vector<int64_t> &dims,
  ...
  printf("Filter_box size %ld", filtered_box.size());
  return infer_rec(filtered_box, origin);
}

std::vector<cv::Mat> OCR_PPredictor::det_crop_imgs(const std::vector<int64_t> &dims,
                                                   const float *input_data, int input_len, int net_flag,
                                                   cv::Mat &origin) {
    PredictorInput input = _det_predictor->get_first_input();
    input.set_dims(dims);
    ...
    std::vector<cv::Mat> imgResults;
    for (auto bp = filtered_box.crbegin(); bp != filtered_box.crend(); ++bp) {
      const std::vector<std::vector<int>> &box = *bp;
      cv::Mat crop_img = get_rotate_crop_image(origin, box);
      crop_img = infer_cls(crop_img);
        imgResults.emplace_back(std::move(crop_img));
    }
    return imgResults;
}
...
std::vector<OCRPredictResult> OCR_PPredictor::infer_rec(
  ...
  printf("ocr_results finished %lu", ocr_results.size());
  return ocr_results;
}
...
cv::Mat OCR_PPredictor::infer_cls(const cv::Mat &img, float thresh) {
  ...
  for (int64_t i = 0; i < results.at(0).get_size(); i++) {
    printf("output scores [%f]", scores[i]);
    if (scores[i] > score) {
      score = scores[i];
      label = i;
    }
...
float OCR_PPredictor::postprocess_rec_score(const PredictorOutput &res) {
  ...
  if (count == 0) {
    printf("calc score count 0");
  } else {
    score /= count;
  }
  printf("calc score: %f", score);
  return score;
}
